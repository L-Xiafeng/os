...
struct gdt_desc {
   uint16_t limit_low_word;//16 bit
   uint16_t base_low_word;//16 bit
   uint8_t  base_mid_byte;//8 bit
   uint8_t  attr_low_byte;//8 bit
   uint8_t  limit_high_attr_high;//8 bit
   uint8_t  base_high_byte;//8 bit
}; 
...
#define NULL ((void*)0)
#define DIV_ROUND_UP(X, STEP) ((X + STEP - 1) / (STEP))//向上取整X/STEP 的上取整。
#define bool int
#define true 1
#define false 0
