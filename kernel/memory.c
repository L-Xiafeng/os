...
static void vaddr_remove(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt){
    ...
    if (pf == PF_KERNEL)
    {
        ...
        while (cnt < pg_cnt)
        {
            bitmap_set(&cur->userprog_vaddr.vaddr_bitmap, bit_idx + (cnt++), 0);
        }
    }
...
/* 得到虚拟地址vaddr对应的pte指针
 * 对pte_ptr进行操作会直接操作vaddr所在的页，pte_ptr指向一个pte(page)
 */
uint32_t* pte_ptr(uint32_t vaddr) {
    /* 先访问到页表自己(最后一个[即第1023个,0x3ff]页目录项保存的是页目录表的物理地址),此处CPU将实际上的PD当作下一步的PT + 
    * 再用vaddr中的PDE索引来u访问对应的PDE(PT) + \
    * 再用pte的索引做为PT内偏移得到vaddr所在的页*/
    uint32_t* pte = (uint32_t*)((0x3ff<<22) + ((vaddr & (0x3ff<<22) ) >> 10) + PTE_IDX(vaddr) * 4);
    return pte;
}

/* 得到虚拟地址vaddr对应的pde的指针
 * 对pde_ptr进行操作会直接操作vaddr所在的页表,pde_ptr指向一个PT
 */
uint32_t* pde_ptr(uint32_t vaddr) {
   /* 0xfffff是用来访问到页表本身所在的地址
    * 0xfffff 高10位为0x3ff，低10位也是0x3ff
    * 1,CPU先将PD当作PT
    * 2,CPU将PD当PTE
    * 3,用vaddr内PDE的索引，从PD中得到对应的PDE(地址)
    */
...
/************************   注意   *************************
 * 执行*pde,会访问到空的pde。所以确保pde创建完成后才能执行*pde,
 * 否则会引发page_fault。因此在*pde为0时,*pde只能出现在下面else语句块中的*pde后面。
 * *********************************************************/
   ...
   if (*pde & 0x00000001) // 页目录项和页表项的第0位为P,此处判断目录项是否存在
   {
        ...
        if (!(*pte & 0x00000001))// 只要是创建页表,pte就应该不存在,多判断一下放心
       {
        ...
        uint32_t pde_phyaddr = (uint32_t)palloc(&kernel_pool);//从内核分配一个页表PT(PDE)的空间
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt){
    ...
    uint32_t page_cnt = 0;
    ASSERT(pg_cnt >=1 && vaddr % PG_SIZE == 0); //vaddr必须是一个完整的页
...
static void mem_pool_init(uint32_t all_mem){
    ...
    uint32_t page_table_size = PG_SIZE * 256;   // 页表大小= 1页 的页目录表+第0和第768个页目录项指向同 一个 页表+
                                                // 第769~1022个页目录项共指向 254个 页表,共256个页框
    uint32_t used_mem = page_table_size +0x100000; //0x100000为低端1M内存，在之前的过程中被内核占用
    uint32_t free_elem = all_mem - used_mem; 
    uint16_t all_free_pages = free_elem/PG_SIZE; // 1页为4k,不管总内存是不是4k的倍数,对于以页为单位的内存分配策略，不足1页的内存不用考虑了。
...
void mem_init() {
   ...
   uint32_t mem_bytes_total = (*(uint32_t*)(0xb00));//32M
